...
function customizer_library_vogue_build_styles() {
	...
	$setting = 'vogue-nav-font-size';
	$mod = get_theme_mod( $setting, customizer_library_get_default( $setting ) );

	if ( $mod !== customizer_library_get_default( $setting ) ) {

		$nav_text_size = esc_attr( $mod );
...
function customizer_library_vogue_styles() {
	...
	if ( ! empty( $css ) ) {
		wp_register_style( 'vogue-customizer-custom-css', false );
		wp_enqueue_style( 'vogue-customizer-custom-css' );
		wp_add_inline_style( 'vogue-customizer-custom-css', $css );
	}
}
endif;
